...
class BaseFactory():
    ...
    SUPPORTED_AUDIOFILES = ['MP3', 'WAV', ] # 'MP4', 'FLAC', 'AIFF', 'OGG', 'AAC', 'AC3']
...
class StartSettingInspector(BaseFactory):
    ...
    def loadMetadata(self, filepath:str=None) -> bool:
        ...
        match fileExt:
            ...
            # currently only mp3 and wav supported
            # case '.mp4':
    ...
    def loadMetadataMP3(self) -> dict:
        ...
        metadata = {'format': 'MP3',
                    ...
                    'filesize': os.path.getsize(self.filepath) / (1_000 * 1_000)
                    }
    ...
    def validateUserInput(self, **kwargs) -> bool:
        ...
        userInputIsValid = self.isCorrectFile() and self.loadMetadata() and self.hasAtLeast320kbps() and self.correctCheckboxes()
    ...
    def hasAtLeast320kbps(self) -> bool:
        # only 320kbps files make sense to process:
        if self.metadata['format'] == 'MP3' and self.metadata['bitrate'] < 320:
            errorMsg = f'The selected MP3 file should have a bitrate of 320kbps, found bitrate of {self.metadata["bitrate"]}kbps.'
            modelLogger.warning(errorMsg)
